import { ShieldCheck, Database, Lock, Eye, Trash2, Mail, Globe, Server } from 'lucide-react';
  ...
  return (
    <div>
      ...
      <div className="mb-10 p-6 rounded-3xl bg-white dark:bg-[#1A1917] border border-[#E0DBD3] dark:border-[#2E2C29]">
        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#B5B0A8] mb-3">TL;DR — Short version</p>
        <div className="grid sm:grid-cols-3 gap-4">
          {[
            { icon: '🚫', text: 'We do not sell or share your personal data with third parties for advertising.' },
            { icon: '🔒', text: 'Your flashcard collections are stored in Google Firebase — encrypted in transit and at rest.' },
            { icon: '🗑️', text: 'You can export or permanently delete all your data at any time from the app.' },
          ].map((item, i) => (
            <div key={i} className="flex items-start gap-3">
              <span className="text-xl shrink-0">{item.icon}</span>
              <p className="text-[12px] text-[#5A5650] dark:text-[#9A9590] font-medium leading-snug">{item.text}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <Section title="1. Who We Are" icon={Globe}>
        <P>
          Kolo Sets ("Kolo", "we", "us", or "our") is a web application for learning the Norwegian language using flashcards,
          spaced-repetition, and quiz modes. It is developed and operated by <strong className="text-[#1A1714] dark:text-[#F0EDE8]">Euphoria Software</strong>,
          trading as <strong className="text-[#1A1714] dark:text-[#F0EDE8]">Dakuta</strong>.
        </P>
        <P>
          This Privacy Policy describes how we collect, use, and protect information about you when you use the Kolo Sets
          web application at <span className="font-mono text-[#FF5733]">kolo.dakuta.dev</span> and any related services.
        </P>
      </Section>
      ...
      <Section title="2. What Data We Collect" icon={Database}>
        ...
        <div className="space-y-4">
          <div>
            <p className="font-black text-[#1A1714] dark:text-[#F0EDE8] mb-1.5 text-[13px]">Account Information</p>
            <UL items={[
          ...
          <div>
            <p className="font-black text-[#1A1714] dark:text-[#F0EDE8] mb-1.5 text-[13px]">Learning Data</p>
            <UL items={[
          ...
          <div>
            <p className="font-black text-[#1A1714] dark:text-[#F0EDE8] mb-1.5 text-[13px]">Technical Data (via Firebase)</p>
            <UL items={[
          ...
          <div>
            <p className="font-black text-[#1A1714] dark:text-[#F0EDE8] mb-1.5 text-[13px]">Feedback Data</p>
            <UL items={[
        ...
        <Highlight>
          We do <strong>not</strong> collect: precise location, device sensors, contacts, cookies for tracking, advertising identifiers, or any biometric data.
        </Highlight>
      </Section>

      {/* 3. How we use it */}
      <Section title="3. How We Use Your Data" icon={Eye}>
        <P>We use your data solely to provide and improve the Kolo Sets service:</P>
        <UL items={[
        ...
        <P>
          We do <strong className="text-[#1A1714] dark:text-[#F0EDE8]">not</strong> use your data for advertising,
          profiling, or sell it to any third party.
        </P>
      </Section>
      ...
      <Section title="4. Data Storage & Security" icon={Server}>
        <P>
          All user data is stored in <strong className="text-[#1A1714] dark:text-[#F0EDE8]">Google Firebase</strong>{' '}
        ...
        <P>
          Firebase data is hosted in the <strong className="text-[#1A1714] dark:text-[#F0EDE8]">European Union (eur3 region)</strong> to comply with GDPR data residency requirements.
        </P>
        <P>
          Additionally, some data (collections) is cached in your browser's <span className="font-mono text-[13px]">localStorage</span> for offline access and performance. This data never leaves your device unless you choose to sync.
        </P>
      </Section>
          ...
          {[
            {
              ...
              purpose: 'Authentication and database (Firestore). Your email, name, and all learning data is stored here.',
              link: 'firebase.google.com/support/privacy',
            },
            {
              name: 'Telegram',
              purpose: 'Receiving support feedback messages via the feedback form. Only messages you explicitly send through the Support page are transmitted.',
              link: 'telegram.org/privacy',
            },
          ].map(item => (
            <div key={item.name} className="p-4 rounded-2xl bg-[#EDEAE4] dark:bg-[#242220]">
              <p className="font-black text-[#1A1714] dark:text-[#F0EDE8] text-[13px] mb-1">{item.name}</p>
              <p className="text-[12px] mb-1.5">{item.purpose}</p>
              <p className="text-[11px] font-mono text-[#B5B0A8]">{item.link}</p>
            </div>
          ))}
        </div>
        <P>We do not use Google Analytics, Meta Pixel, or any other advertising/tracking technology.</P>
      </Section>

      {/* 6. Your rights */}
      <Section title="6. Your Rights (GDPR)" icon={ShieldCheck}>
        <P>
          If you are located in the European Economic Area (EEA), you have the following rights under the
        ...
        <P>
          ...
          <a href={`mailto:${CONTACT_EMAIL}`} className="text-[#FF5733] hover:underline font-mono">{CONTACT_EMAIL}</a>.
      ...
      <Section title="8. Children's Privacy" icon={ShieldCheck}>
        <P>
          Kolo Sets is not directed at children under the age of 13. We do not knowingly collect personal information
          from children under 13. If we discover that a child under 13 has provided us with personal data, we will
          promptly delete it. If you believe your child has provided us with their data, please contact us at{' '}
          <a href={`mailto:${CONTACT_EMAIL}`} className="text-[#FF5733] hover:underline font-mono">{CONTACT_EMAIL}</a>.
        </P>
      </Section>
      ...
      <Section title="9. Changes to This Policy" icon={Mail}>
        <P>
          ...
          top of this page and, if changes are material, notify active users via an in-app notice.
        </P>
        <P>
          Continued use of Kolo Sets after changes take effect constitutes your acceptance of the updated policy.
        </P>
      </Section>
      ...
      <Section title="10. Contact Us" icon={Mail}>
        ...
        <div className="p-4 rounded-2xl bg-[#EDEAE4] dark:bg-[#242220]">
          ...
          <p className="text-[13px] mt-1">
            Or use the{' '}
            <Link to="/support" className="text-[#FF5733] hover:underline">in-app support form</Link>
