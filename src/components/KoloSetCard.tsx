...
import { Layers, ChevronRight, X, BookOpen, Zap, Plus, Check, Quote } from 'lucide-react';
import { KoloSet, CEFRLevel } from '../types/kolo-sets';
import { createCollection, updateCollection, createCard } from '../utils/storage';
...
const createCollectionWithCards = async (name: string, language: string, set: KoloSet) => {
...
export default function KoloSetCard({ set }: KoloSetCardProps) {
  ...
  const handleStartStudy = async (mode: 'flashcards' | 'quiz') => {
    ...
    const collection = await createCollectionWithCards(set.title, set.language, set);
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1.5">
              <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-widest ${LEVEL_COLORS[set.level]}`}>
                {set.level}
              </span>
              <span className="text-[11px] text-gray-400 dark:text-gray-500">{set.topic}</span>
            </div>
          ...
          >
            ...
            <div className="flex justify-between items-start p-6 border-b border-gray-100 dark:border-gray-700/50 shrink-0">
              <div>
                ...
                <h3 className="text-xl font-bold text-gray-900 dark:text-white">{set.title}</h3>
                <p className="text-sm text-gray-400 dark:text-gray-500 mt-0.5">{set.cardCount} слов</p>
              </div>
              <button
                onClick={() => setShowWordsModal(false)}
            ...
            <div className="p-4 border-t border-gray-100 dark:border-gray-700/50 shrink-0 flex gap-2">
              <button
                onClick={() => { setShowWordsModal(false); setShowModeModal(true); }}
